...
`RequireVerifiedContributorForMaterialUploads` used to live here and went with the model it gated
(`MaterialSubmission`). The flag it read, `material_uploads_verified_only`, is very much alive — it
is checked in `coauthoring.services._require_verified_contributor_for_uploads`, which carries the
whole of this class's reasoning (inverted semantics, a deliberate fail-CLOSED default) plus the one
that moved it out of a permission class at all: the restriction applies to one SHAPE of request, a
version that carries a file, and a permission class cannot see which of a version's three payloads
arrived."""
