...
**The published check is the one rule that is stricter here than on `CourseItem` itself.**
`LessonExerciseSet.visible_exercises` drops unpublished exercises for anybody who cannot curate, and
its own docstring says plainly that `CourseItem` "does not make this check and should" — an exercise
is unpublished when a moderator has pulled it, usually because its solution is wrong, which is the
last thing to hand somebody searching for homework. Search honours it (for materials too, on the same
reasoning) rather than reproducing a known gap in a new surface. Fixing `CourseItem` itself is a
change to what the course page renders and belongs with that page, not smuggled in here.
