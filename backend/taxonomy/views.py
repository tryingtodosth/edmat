...
#:
#: The size of that is not theoretical: measured against the real corpus on 2026-09-19,
#: `/api/disciplines/{slug}/branches/` ran 149 queries for one request. That is the most expensive
#: anonymous request in the app, because the frontend's root layout preloads the taxonomy on EVERY
#: page load — and the response cache only starts serving a URL after its second miss, so a
#: first-time visitor always pays it in full.
_BRANCH_DETAIL_PREFETCH = (
    'translations',
    'topics__translations',
    'chapters__translations',
    'chapters__topics',
)
...
class BranchViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def exercises(self, request, slug=None):
        ...
        from exercises.views import _browse_exercises, _filter_exercises, sort_exercises
