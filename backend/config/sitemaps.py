...
* Event         — `status='published'` AND `visibility` in `PUBLIC_VISIBILITY`. `cancelled` is
                  publicly readable (so the link in the cancellation notice works) but must never
                  be advertised: it is an invitation to something that is not happening. And an
                  event now defaults to `private`, which is reachable-with-a-link and must not be
                  listed — the same distinction `courses` draws.
...
from events.models import PUBLIC_VISIBILITY, Event
...
class EventSitemap(_FrontendSitemap):
    ...
    def items(self):
        ...
        # BOTH conditions, and the second one post-dates the rest of this module: `visibility` was
        # added to Event after this file was first written and defaults to `private`, so filtering
        # on `status` alone would advertise every unlisted event to a crawler. Same shape as
        # CourseSitemap above, and the same reason the events app keeps its own frozenset.
        return Event.objects.filter(
            status='published', visibility__in=PUBLIC_VISIBILITY
        ).order_by('pk')
