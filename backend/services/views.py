...
class ServiceViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        qs = (
            ...
            # Explicit, and NOT redundant with `Service.Meta.ordering`: an `annotate()` carrying an
            # aggregate CLEARS the model's default ordering (Django does this so a default ordering
            # cannot silently join the GROUP BY and change the grouping). Without this line the two
            # annotations above quietly reverse the browse page from newest-first to oldest-first —
            # measured, not guessed: the listing came back 1,2,3… where it had been 18,17,15….
            .order_by('-created_at')
        )
