...
# Integration (CONFERENCE-BRIEF.md §5): the door answers the same 409 `briefing_unread` as the check-in
# button and the cloakroom desk until the scanning volunteer has read the mandatory documents.
...
class TicketMixin:
    ...
    def _post_scans(self, event, request):
        ...
        if not event.is_staff_member(request.user):
            return Response(status=status.HTTP_403_FORBIDDEN)
        if request.method == 'POST':
            block = briefing_block_reason(event, request.user)
            if block:
