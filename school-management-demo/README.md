...
| `docs/EVENTS.md` | wydarzenia szkolne od strony organizatora (3.10): czym różnią się od wycieczki (3.8) i od kalendarza rodzica (3.7.8); pojemność użytkowa sali a limit przeciwpożarowy (ten drugi blokuje zatwierdzenie); wolontariat małoletnich w kolejności zgoda → instruktaż → dyżur, z dobowym limitem, ciszą nocną, przerwą i dorosłym na stanowisku (zapis czeka, zamiast zostać odrzucony); wejściówka z nieprzezroczystym tokenem i bramka działająca bez sieci (`nonce`, duplikat vs kolizja); catering wyłącznie w liczbach (14 alergenów EU FIC); rejestr ryzyka 5×5 blokujący zatwierdzenie; dane ulotne znikają po 30 dniach, liczby zbiorcze zostają |
