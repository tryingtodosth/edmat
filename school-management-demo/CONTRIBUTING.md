...
| `schoolEvents` | `{ id, name, kind:'openDay'|'fete'|'ceremony'|'parentEvening'|'competition'|'other', date, start, end, leaderId, meetingId, objective, outcome, measure, redLines:[], audience:[], expectedVisitors, rooms:[{roomId,name,layout,areaM2,bufferPct,fireCapacity,expected,note}], status:'draft'|'submitted'|'approved'|'closed', createdAt, approvedBy, approvedAt, purgedAt, summary }` (owner: 3.10; the organiser's side of an open day/fête — **not** a trip (`trips`, 3.8) and not the parent's calendar (`meetings`, 3.7.8); `meetingId` links to that calendar row) |
| `eventShifts` | `{ id, eventId, station, date, start, end, needed, adultRequired, assignees:[{kind:'staff'|'student', userId, studentId, status:'confirmed'|'pending', pendingReason, minor, at, byUserId}], note, createdAt }` (owner: 3.10; a minor's claim stays `pending` until an adult is on the same shift) |
| `eventBriefings` · `eventBriefingAcks` | `{ id, eventId, title, tier:'public'|'participant'|'staff'|'organizer', body, requiresAck, version, at, byUserId }` · `{ id, briefingId, eventId, userId, studentId, version, at }` (owner: 3.10; republishing the same title bumps `version` and invalidates every acknowledgment) |
| `eventConsents` | `{ id, eventId, studentId, scope:'volunteer', byUserId, by, at, note }` (owner: 3.10; the guardian's consent **and** the pupil's enrolment — it is what puts them in the `staff` briefing tier) |
| `eventPasses` · `eventScans` | `{ id, eventId, token, kind, label, issuedAt, issuedBy, revoked, inside }` · `{ id, eventId, passId, token, direction:'in'|'out', at, nonce, deviceId, status:'ok'|'duplicate'|'collision'|'unknown'|'revoked'|'rejected', code, byUserId }` (owner: 3.10; `token` is 32 URL-safe random chars and carries **no** PII; the client's `nonce` makes a replayed offline batch idempotent) |
| `eventDietary` · `eventRisks` | `{ id, eventId, passId, diet, allergens:[], at, byUserId }` · `{ id, eventId, hazard, category, likelihood:1-5, severity:1-5, control, fallback, ownerId, at, byUserId }` (owner: 3.10; dietary data leaves the module **only** as aggregates, and a risk scoring above `config.events.riskAcceptMax` blocks approval) |
