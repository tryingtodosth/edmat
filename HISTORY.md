...
The full chronological build log: every feature, the reasoning behind each design decision, every
bug found and how it was found. Split out of `CLAUDE.md` on 2026-09-19, verbatim — nothing here was
rewritten, only moved.

**Read this when you need to know *why* something is the way it is.** For how to work in the code
today, read `CLAUDE.md` (root), then `backend/CLAUDE.md` / `frontend/CLAUDE.md` and the per-app
files. `CLAUDE_MAP.md` is the orientation map.

## Two staleness warnings, because this file is a record and not a spec

1. **The taxonomy was renamed after most of this was written.** `taxonomy.Field` → `Discipline`,
   `taxonomy.Course` → `Branch`, and the `classroom` app → `courses`. Entries below use whichever
   names were current when they were written. `backend/CLAUDE.md` has the mapping table.
2. **A "Left open, not built" list is only true as of its own entry.** Several were closed by later
   entries; where that happened, the later entry says so. When an old note and a newer one disagree,
   the newer one wins.

Status marks: ✅ exists today · 🆕 planned, not built · ⚠️ a real open decision.

---

## The running status paragraph, as it stood at the split

This is the summary that used to sit at the top of `CLAUDE.md` and grew by a sentence per feature.
It stops here (the sections below carry on past it); it is kept because it is the only place that
reads the first two years as one paragraph. Test counts are as of whichever pass wrote them.
...
## The one-time corpus migration (Section 12 of the old blueprint)
...
## 17AX. The material's own button moved, pictures shown, tabs named, messages encrypted (✅ built)

Four asks in one go, three of them small and one of them a real change to what is stored.

### The button that gets you the material

It sat at the bottom-right of the card footer, after the claim chips, the tags, the price and the
attribution line. Somebody who has read what a material is has exactly one next question, and the
answer to it was below everything else and pushed to the far edge, where it read as one more piece
of metadata rather than the card's own primary action. It now sits directly under the title and the
summary, left-aligned with the text column, in its own `.material-card__get` row — on the browse
cards and the detail page alike, since it is the same component and the same question in both.

The two labels stay as they were: **Download** for a file this site hosts, **Open** for a link
somebody else does. `download` on an anchor pointing at another site's page is a claim about what
clicking does that would not be true.

### Pictures

A material could already BE a picture — `.png`/`.jpg` have always been accepted, and every uploaded
image is re-encoded and stored as `.webp` (`materials/materialfile.py`) — and nothing ever showed
it. The detail page previewed PDFs only, so a photographed exam sheet was a file you had to download
to find out what it was.

It now renders inline, directly under the card, and **not** behind a toggle the way the PDF preview
is. The two are not the same decision: pdf.js costs a ~1.5 MB script chunk before it can draw
anything, while an `<img>` costs one request for a file the browser was built to render. Bounded to
80vh as well as 100% width — a tall scan would otherwise push the claims, reviews and discussion off
the bottom of a phone — and wrapped in a link to the unbounded original. A white backdrop, because a
re-encoded WebP keeps its alpha and a diagram scanned as dark-on-transparent is invisible in dark
mode. `.webp` also joined the two upload pickers' `accept` lists, which had been missing the one
extension every stored picture actually has.

### Every tab is named `EdMat: …`

A tab strip gives a page perhaps a dozen characters before it truncates, and with the app name last
those characters were spent on the route while the part that tells one EdMat tab from another was
cut. 41 routes wrote `{page} — {m.common_appName()}` by hand; they now call `pageTitle()`
(`lib/utils/pageTitle.ts`).

**`PageHead.svelte` already owned this for nine other routes** and is now the same function's other
caller, rather than a second definition of the shape. That is worth recording because the first pass
missed it and added a SECOND `<title>` to each of those nine — two title elements in one document is
not an error anybody is told about; the first one silently wins, and every one of those pages would
have been stuck on one name forever. Found by grepping the built HTML, which is why the browser
script now asserts `head title` count is exactly 1 on every page it visits.

The homepage is `EdMat: Home` rather than a bare `EdMat`: `PageHead` still supports the no-title case
(a detail page whose record has not loaded yet gets the app name alone rather than a stray colon),
but a homepage is a page like any other and a row of tabs should be able to tell it apart.

### Messages are encrypted at rest

`messaging/crypto.py`, AES-256-GCM, `edmat1:<base64url(nonce‖ciphertext‖tag)>`. Encrypted in
`services.send_message`/`reply_to_message` — the app's only two write paths — and decrypted in
`MessageSerializer`, its only read path.

**What it defends against, and what it does not.** Until now every private message sat readable in
one file on a shared university host, present in every backup anybody takes. Encrypting the body
means the file alone is no longer enough. The key lives on the same server, because the server has
to hand the message back to the two people it belongs to — so anybody who can run code there can
read it, and so can whoever operates the machine.

**Not end-to-end, deliberately rather than by omission.** E2EE is the stronger answer and it changes
the product: keys in the browser mean losing the browser loses every message with no recovery this
project could offer; a reported message cannot be read by the moderator deciding the report; and the
guardian oversight of §17AP would be unimplementable for messages. Those are decisions for whoever
owns this project, not details to settle inside a crypto module — and until they are settled, this
is the change that is a strict improvement on plaintext with nothing lost.

**The body and not the subject**, also deliberately. `postman.Message.subject` is a `CharField(120)`
and the ciphertext of an N-byte plaintext is N+28 bytes before base64 — so encrypting into that same
column would cut what somebody may type from 120 characters to about 56 bytes, fewer in Polish where
most non-ASCII letters cost two. What that buys is small: a subject here is usually a tutoring
listing's own title, carried over by the Contact link from a page that is already public. Widening
the column is the honest way to do the subject too, and it is a schema change to a third-party app's
table. (One real bug fixed next door: the compose form's `maxlength` was 255 against that same
120-character column, so a long subject was refused by the API with a 400 nobody could have
predicted.)

**A mixed table is a normal state.** The version prefix is what makes that work — `decrypt_text`
returns an unprefixed value unchanged, so rows written before this landed, or through the Django
admin, keep reading. `manage.py encrypt_messages` converts them when somebody chooses to, and is
idempotent; it is deliberately NOT a data migration, which would encrypt every row under whatever
key happened to be resolved at migrate time on every deployment, including one whose
`EDMAT_MESSAGE_KEY` was not set up yet. Getting that wrong makes messages unreadable rather than
merely unencrypted, and the failure surfaces long after the migration that caused it.

**An unreadable row does not take the inbox with it.** A rotated key or a damaged value raises
`MessageDecryptionError`, which the serializer turns into `body: ''` plus `body_unavailable: true`,
rendered as a sentence in the reader's own language. The decrypt is cached on the ROW, not on the
serializer — DRF reuses one child serializer across a list, and caching on `self` would serve the
first message's body for every message in an inbox (the bug §17F already recorded once for
`ExerciseDetailSerializer`; a test with two different bodies is what catches it).

`EDMAT_MESSAGE_KEY` (base64url of 32 bytes) is the key; unset, it is derived from `SECRET_KEY` via
HKDF so a fresh clone works with no setup step — which carries the cost that rotating the secret key
then orphans every message, stated in `settings.py` beside the setting rather than left to be
discovered. `cryptography` is a new dependency: the standard library has no authenticated cipher,
and hand-rolling one is the classic way to end up with something that looks encrypted.

### Verified

**Backend: 20 new tests** (`messaging/test_encryption.py`), and the 16 that already existed are now
a real round-trip regression check for free. **Full suite afterwards: 1498 tests, OK, zero failures**
(~21 minutes). Worth knowing before reading that output: the run prints a great many
`DatabaseOperationForbidden: Database queries to 'logs_N' are not allowed in this test` tracebacks,
which are **not failures** — the telemetry request-log middleware runs under the test runner, tries
to write a row to a shard the test class has not declared, and its own `except Exception` logs the
traceback and carries on. They are pre-existing noise that buries anything real; `grep -cE
'^(ERROR|FAIL): '` is the number that matters. Weighted at what fails silently: a body that is not
actually encrypted still round-trips perfectly through the API, so every test that matters reads the
DATABASE ROW rather than the response. Covered: the stored column does not contain the words; both
people read them back; a reply too; the subject deliberately untouched (pinned, so changing it is an
act rather than an accident); an empty body left alone; a list decrypting every row and not just the
first; a legacy plaintext row still readable; the command converting it and being idempotent;
`--check` changing nothing; a row sealed under another key surfacing as unavailable with a 200 on
both retrieve and list; tampering detected rather than decrypting to something else; a wrong-length
key refused loudly.

**Browser: `e2e/material-open-and-titles.mjs`, 33 checks, zero console/page errors** — the button
below the title and the summary, at the summary's own left edge, in the card's left half, above the
claim groups, still a 44px target; a picture material's preview GENUINELY LOADED (`naturalWidth > 0`,
not merely present — a broken `src` passes a selector check and fails a reader); a PDF still getting
its collapsed toggle and no picture block; six pages named `EdMat: …` with exactly one title element
each; a detail page named after its own record; and a message sent through the real compose form,
read back word for word by its recipient through the real thread page. The half a browser cannot see
— that the column holds `edmat1:…` and not those words — was checked directly against the database
afterwards.

Screenshots looked at, which is how the layout above was confirmed rather than assumed.

`manage.py encrypt_messages` was then run for real against this checkout's own four existing dev
messages — 4 converted, a second run reporting nothing left to do, and both still reading back
correctly afterwards. That is the command exercised on real data rather than only on fixtures.

### Left open, not built

- **The subject is not encrypted** — see above. It needs the column widened first.
- **No E2EE**, and the three product decisions it would force are untouched.
- **The Django admin shows ciphertext** for a message body. Arguably right — staff have no business
  casually reading private messages — but it is a side effect of this change rather than a decision
  anybody made, and postman's own admin is registered.
- **Nothing encrypts a message's existence or its metadata**: who wrote to whom, when, and about
  what subject are all still plainly readable in the row.
- **No UI anywhere says messages are encrypted**, on purpose. The true sentence needs three
  qualifications after it (not end-to-end, staff can read them, the subject is in clear), and a
  privacy claim a reader cannot check is worse than no claim. Worth writing properly if it ever goes
  on a page.
- **The SPA fallback (`build/200.html`) still has no `<title>` at all**, so a route that is not
  prerendered shows the URL in the tab until the bundle boots. A static one in `app.html` is the
  obvious fix and was NOT taken on a guess: it would be a second title element on every prerendered
  page, and whether the browser keeps using the first one after hydration decides whether that is
  harmless or freezes all 41 of the others. Worth doing, against a static serve of `build/` where it
  can actually be observed.
- **A long claim chip still overflows the material card's right edge**, and a long material title
  collides with its type badge on a grid card (both visible on `/materials`) — pre-existing,
  untouched here, and worth a look.
- **`e2e/material-types.mjs` wants a fixture account (`zzz-type-student@example.test`) that this
  checkout does not have**, so it fails at sign-in before doing anything. Unrelated to this work and
  not fixed here. (An earlier draft of this section claimed the seeded demo passwords were wrong —
  they are not. That probe posted `{"email": …}` to a login endpoint whose field is `username`,
  which answers 401 for a perfectly good password. The correction, and the two other sign-in traps
  this run found, are in `test.md`.)

---

## 17AY. Pictures on a piece of content, and applying to look after them (✅ built, full stack)

Two features asked for together, and they interlock: **anybody signed in may add a picture; putting
them in order is a job somebody has to be trusted with, and applying is how a reader becomes that
person.** A third part was asked for and is deliberately absent — see the end.

### The gallery

A new `galleries` app. `Gallery` (one per piece of content, a generic target) and `GalleryImage`
(the picture, a caption, a curated `order`, its uploader).

**Not a comment attachment.** An attachment belongs to one person's remark and reads as part of the
sentence they wrote. A gallery belongs to the CONTENT — the twelve photographed pages of a handout,
the diagrams a proof needs — and is the same set whoever is reading. Giving them one model would
mean every listing of either having to explain which kind it was looking at.

**One gallery per piece of content, not one per person.** The other obvious shape is worse here:
five people who each photographed a different page of the same handout would produce five one-page
galleries, and the reader — who wants the handout — would have to open all five and work out the
order themselves. So an image carries its own uploader while the ORDER belongs to the gallery, which
is precisely what makes curating it a real job rather than a preference.

**Three targets** (`material`, `exercise`, `post`), in a deliberately separate and much shorter
registry than `community/targets.py`'s comment targets. A thread makes sense on a coverage claim or
a review; a photo album does not, and inheriting the longer list would mean every new comment target
silently becoming somewhere pictures could be uploaded.

**A gallery is exactly as visible as its target**, written as one predicate per model in
`galleries/visibility.py` rather than a generic `published` check — a generic check silently passes a
model that happens not to have the field, and the failure mode is leaking an unpublished exercise
through the pictures attached to it. An unknown target type is not readable rather than readable by
default. A dedicated test covers exactly that leak.

**The picture is never the bytes that were uploaded**: the shared `imaging` pipeline (sniff, byte
cap, decompression-bomb budget, decode, EXIF dropped by re-encoding) plus an aspect-preserving bound
at 2000px. Bigger than the 1600 a feed picture gets, and not imported from there: a gallery picture
is routinely a photographed page somebody needs to read the handwriting on, which is the case 1600
starts to fail. Shrink-only, so a small picture is not blown up. Centre-cropping (what an avatar
gets) is wrong here — on a photographed exercise sheet the margins are where half the working is.
Dropping EXIF also stops a photograph of a lecture hall publishing the GPS fix of the room.

Bounded three ways: 10 MB per upload, 40 pictures per gallery, and the same per-account byte
allowance material submissions and comment attachments already share — one account, one budget,
however the bytes got there. Its own throttle scope, because every call decodes and re-encodes a
real image. A picture from a child's account, or on content aimed at children, is held for a
moderator through the existing `hold_for_review` — the same rule and the same queue as a picture on
a comment. Reportable as `gallery_image`; the model carries the `auto_hidden_at`/`is_removed` pair
the report machinery already looks for, so restore and remove work unchanged. No viewer pool, so
auto-hide never fires on its own and reports gather for a human — the Service posture. Behind a
`galleries` kill switch.

**Reordering takes the whole list, not one move.** That is the shape a list naturally submits, and
it cannot leave two pictures claiming the same position the way a sequence of single moves can. A
partial list is refused rather than topped up: a client that sent one is working from a stale
gallery, and inventing positions for what it forgot is how two people reordering at once produce an
order neither chose.

### Applying to look after something

`moderation.GovernorApplication`, beside `NodeGovernor` rather than in an app of its own — an
application is a request for exactly that row.

**A Material is now a governable node**, joining Discipline and Branch. Before galleries there was
nothing a person could usefully be trusted with at the level of a single material, so the smallest
unit of authority was a whole branch. A gallery gives one, and asking somebody to take on a branch in
order to order twelve photographs would grant far more than the job needs. The three levels nest —
a grant on a material, its branch, or that branch's discipline all answer yes for the material —
resolved by `is_governor_of_material`, one level deeper than the existing cascade.

That widening needed one real change to `NodeGovernorSerializer`: a material is addressed by `node_pk`
rather than `node_slug`, because `Material.slug` is only unique WITHIN a branch, and a lookup by slug
alone would silently pick whichever row the database returned first.

**The queue is first come, first served, and there is deliberately no way to change that** — no
priority column, no fee, and no hook for one. What an applicant gets instead is the truth about where
they stand: `queue_position`, shown on the material, in Settings, and in the queue, next to a sentence
saying nobody can be read sooner. That is the half of "transparent queue tracking" that was actually
worth having.

**Deciding stays with staff**, unchanged from §17M, even though the hierarchy now runs three deep and
letting a branch governor approve applications for materials inside it is the obvious next step.
Delegated granting has real failure modes (a ring of mutual approvals) and is not this pass's to
invent. Approving creates the real grant in one place, so there is no path where somebody is told yes
without the authority existing; an existing grant is reused rather than duplicated, since staff may
well have granted it by hand while the application waited. A decline requires a reason. The claim is
a WHERE-anchored `update()` with a revert on failure — §17I's shape, for §17I's reasons.

Two notification types rather than one, because the recipients are different people with different
next moves: the applicant is told what was decided, and **every** staff account is told there is
something waiting, since a queue that notifies one person stalls the moment that person is away.

### What was asked for and not built

**The paid fast-track.** The request was for an optional fee to be read first, with the money funding
scholarships and volunteer stipends. Piotr dropped it on 2026-09-18 after the objection was put to
him, and the objection is worth keeping: the thing being sold would be **priority for an application
to gain authority over content other people wrote.** Every złoty going to scholarships does not
change how that reads, and it would give the queue an incentive to stay slow. Selling priority for a
volunteer's paid time on something neutral — a transcription, a translation — does not have that
problem; selling it on moderation applications does. This project has also never taken money
anywhere, by decision (`Service.hourly_rate` and `Material.price_amount` are display-only;
AUDIENCE-BRIEF §10 records "no payments"). The model's own docstring carries this, so that a future
reader finds the reasoning where the hook would have gone.

### Verified

**Backend: 53 new tests** — 28 in `galleries/tests.py`, 25 in `moderation/test_governor_applications.py`
— weighted at the refusals. **Full suite afterwards: 1551 tests, OK, zero failures.** An unpublished exercise's gallery is not readable by a stranger; an
unknown target type is not found; a disguised executable is refused; the stored picture is a bounded
WebP with the camera tag gone and a small one not blown up; the per-gallery ceiling and the shared
byte allowance both refuse; somebody who merely uploaded a picture cannot reorder the gallery while a
material-level governor and a branch-level governor both can; a partial order is refused rather than
guessed at; a child's picture waits for a moderator; the kill switch closes the whole surface while
staff still get through; a picture is reportable and a moderator's remove hides it. On the queue: an
empty application is refused, applying twice is refused, the position is the honest one, an applicant
sees only their own, a governor of the branch above cannot decide, declining needs a reason, deciding
twice is a clean 409, and — the interlock, end to end — approving is what makes
`is_governor_of_material` start answering yes.

One pre-existing test needed updating rather than working around: `moderation`'s own
`test_list_is_public_and_returns_all_seeded_flags` asserts the exact set of seeded flags, and there is
now another. That is the intended effect of adding a kill switch, so the expectation was what was
stale — the same note §17T already recorded for the same test.

**One real mistake of my own, caught by looking rather than by a test:** the gallery tests write real
files, and the first version of them wrote into the developer's own `media/galleries/` — 85 stray
WebPs after two runs. That is litter, and worse, it is a way for one run to make the next one's "is
it really gone?" check pass for the wrong reason. Every gallery test now runs against a temporary
`MEDIA_ROOT`, applied per class, which is what `accounts/test_avatar.py` and `events/tests.py`
already do for exactly this reason; confirmed afterwards by running the suite and finding the real
directory empty.

**Browser: `e2e/galleries-and-applications.mjs`, 26 checks, zero console/page errors.** Shaped around
the interlock: a reader adds two real PNGs through the actual form, the thumbnails GENUINELY load
(`naturalWidth > 0`, not merely present), the stored files come back as bounded WebP, she can caption
her own picture and has **no reorder control at all**, she applies through the real dialog, staff find
it in the queue with her words and her name, declining with no reason is refused in words, approving
clears it, and back on the material she is told she looks after it, the reorder buttons are now there,
the order really changes and survives a reload. Then the lightbox opens, steps and closes on Escape,
and a signed-out reader sees the pictures but no upload control and no way to apply. Screenshots
looked at.

### Left open, not built

- **No drag-and-drop reordering** — a pair of up/down buttons instead, which is operable by keyboard
  without a second implementation and is honest for a list that is usually a handful of pages. The
  API takes the whole order, so drag-and-drop is a UI change alone if it is ever wanted.
- **No cover picture** — a gallery has an order and the first one leads; nothing marks one as the
  material's thumbnail, and no browse card shows a picture from a gallery.
- **A governor of a branch cannot decide applications for materials inside it** — see above.
- **No people search**, still — staff granting a role directly (rather than through an application)
  still need the account's numeric id, the gap §17M recorded.
- **Nothing notifies the existing governors of a node** when somebody applies for something under it;
  only staff are told.
- **Galleries are not reachable from anywhere but the content's own page** — no "recently added
  pictures" feed, and the activity feed does not carry them.
- **A withdrawn application can be replaced by a new one**, which starts at the back of the queue.
  That is honest rather than clever, and the position is visible to everybody involved.
...
These were the blueprint's own `## 18. Open questions`. The ones still open live in
`CLAUDE.md` (engineering) and `LEGAL.md` (the corpus copyright question). Kept here in full
because each records a decision and its reasoning, not just an outcome.
...
follows, even where the shapes have moved on. **`CLAUDE.md` supersedes all of it for the present
state**, and the names here are pre-rename throughout (Field → Discipline, Course → Branch).
Section 12 (the corpus migration) and sections 16–17 are above, in their chronological place.
...
**Verified.** `manage.py test events booking notifications moderation`; `manage.py check`;
`makemigrations --check --dry-run`; `npm run check` (0 errors, 0 warnings), `npm run lint`,
`npm run build`; `frontend/e2e/event-tickets.mjs` against the branch's own servers on ports 8104 /
5204, with the ticket-page, scanner and badge-sheet screenshots looked at; en/pl key sets identical
(2748 each). The `tickets` kill switch is asserted from both sides — the whole surface 403s while
check-in by button keeps working.
