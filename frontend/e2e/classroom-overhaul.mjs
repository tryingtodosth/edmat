...
async function openDrawer(page, name) {
	...
	const alreadyOpen = await summary.evaluate(
		(el) => el.parentElement?.hasAttribute('open') ?? false
	);
...
check(
	'but is offered no delete',
	(await coAdmin.getByRole('button', { name: /^Delete$/ }).count()) === 0
);
...
check('and says the submission will be reviewed', /review/i.test(sText), sText.slice(0, 200));

// Material 1 exists in the seeded corpus.
await student.locator('.contribute input[inputmode="numeric"]').first().fill('1');
await student.locator('.contribute button[type="submit"]').first().click();
await settle(student, 2200);
sText = await student.locator('.page').innerText();
check(
	'the contributor is told it is waiting',
	/waiting for review/i.test(sText),
	sText.slice(0, 300)
);
...
check(
	'the queue empties',
	(await coAdmin.locator('.manage__badge').count()) === 0 &&
...
await settle(owner, 1800);
await openDrawer(owner, 'Invite links');
text = await owner.locator('.invites').innerText();
