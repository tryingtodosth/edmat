...
<form onsubmit={submit}>
	<label class="field">
		<span>{m.course_form_title()}</span>
		<input type="text" bind:value={title} maxlength="200" required />
	</label>

	<label class="field">
		<span>{m.course_form_summary()}</span>
		<input type="text" bind:value={summary} maxlength="300" />
		<span class="hint">{m.course_form_summaryHint()}</span>
	</label>

	<label class="field">
		<span>{m.course_form_description()}</span>
	...
	<fieldset class="field choice-set">
		<legend>{m.course_form_visibility()}</legend>

		<label class="choice">
			<input type="radio" bind:group={visibility} value="only_you" />
			<span class="choice__body">
				<span class="choice__label">{m.course_visibility_onlyYou()}</span>
				<span class="choice__hint">{m.course_visibility_onlyYouHint()}</span>
			</span>
		</label>
		...
		<label class="choice">
			...
			<span class="choice__body">
				<span class="choice__label">{m.course_visibility_private()}</span>
				<span class="choice__hint">{m.course_visibility_privateHint()}</span>
			</span>
		</label>
		...
		<label class="choice">
			...
			<span class="choice__body">
				<span class="choice__label">{m.course_visibility_public()}</span>
				<span class="choice__hint">{m.course_visibility_publicHint()}</span>
			</span>
		</label>
	...
	<label class="field">
		...
		<select bind:value={status}>
			<option value="open">{m.course_status_open()}</option>
			<option value="running">{m.course_status_running()}</option>
			<option value="finished">{m.course_status_finished()}</option>
		</select>
		<span class="hint">{m.course_form_statusHint()}</span>
	</label>

	<label class="field">
		<span>{m.course_form_policy()}</span>
		<select bind:value={enrollmentPolicy}>
			<option value="open">{m.course_form_policyOpen()}</option>
			<option value="approval">{m.course_form_policyApproval()}</option>
		</select>
	</label>

	<label class="field">
		<span>{m.course_form_capacity()}</span>
		<input type="number" bind:value={capacity} min="0" />
		<span class="hint">{m.course_form_capacityHint()}</span>
	</label>

	<div class="row">
		<label class="field">
			<span>{m.course_form_startsOn()}</span>
			<input type="date" bind:value={startsOn} />
		</label>
		<label class="field">
			<span>{m.course_form_endsOn()}</span>
			<input type="date" bind:value={endsOn} />
		</label>
	</div>

	<label class="field">
		<span>{m.course_form_price()}</span>
		...
		<span class="hint">{m.course_form_priceHint()}</span>
	</label>

	<fieldset>
		<legend>{m.course_form_discussionLegend()}</legend>
		<label class="field">
			<span>{m.course_form_discussionMode()}</span>
			<select bind:value={discussionMode}>
				<option value="participants">{m.course_form_discussionParticipants()}</option>
				<option value="public">{m.course_form_discussionPublic()}</option>
				<option value="off">{m.course_form_discussionOff()}</option>
			</select>
	...
	<fieldset>
		<legend>{m.course_form_contributionLegend()}</legend>
		<label class="field">
			<span>{m.course_form_contributionPolicy()}</span>
			<select bind:value={contributionPolicy}>
				<option value="approval">{m.course_form_contributionApproval()}</option>
				<option value="open">{m.course_form_contributionOpen()}</option>
				<option value="staff">{m.course_form_contributionStaff()}</option>
			</select>
			...
			<span class="hint">{m.course_form_contributionHint()}</span>
		</label>
	</fieldset>

	<fieldset>
		<legend>{m.course_form_announceLegend()}</legend>
		<label class="check">
			<input type="checkbox" bind:checked={announceNewLessons} />
			<span>{m.course_form_announceLessons()}</span>
		</label>
		<label class="check">
			<input type="checkbox" bind:checked={announceNewPosts} />
			<span>{m.course_form_announcePosts()}</span>
		</label>
		...
		<span class="hint">{m.course_form_announceHint()}</span>
	</fieldset>
