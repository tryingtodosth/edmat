<script lang="ts">
  ...
  import type { Poll, PollOption } from '$lib/types/poll';
  ...
  $effect(() => {
    ...
    loading = true;
    error = null;
    pollsService.getNodePolls(node.kind as any, parseInt(node.id)).then(
      (data) => {
        polls = data;
        loading = false;
      },
  ...
  async function handleCreatePoll() {
    ...
    try {
      const newPoll = await pollsService.createPoll(node.kind as any, parseInt(node.id), createData);
      polls = [...polls, newPoll];
      ...
      error = (err as any).message || 'Failed to create poll';
    }
  }
  ...
  async function handleOpenPoll(pollId: number) {
    try {
      ...
      error = (err as any).message || 'Failed to open poll';
    }
  }

  async function handleClosePoll(pollId: number, decisionNote: string) {
    try {
      const updated = await pollsService.closePoll(pollId, decisionNote);
      polls = polls.map((p) => (p.id === pollId ? updated : p));
    } catch (err) {
      error = (err as any).message || 'Failed to close poll';
    }
  }
</script>
