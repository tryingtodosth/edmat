<script lang="ts">
	...
	import type { CourseStaffMember, StaffRole, TaughtCourse } from '$lib/types/classroom';
	...
	const roleLabels: Record<StaffRole, () => string> = {
		owner: m.classroom_role_owner,
		admin: m.classroom_role_admin,
		assistant: m.classroom_role_assistant
	};
...
<section class="staff">
	<h2>{m.classroom_staff_heading()}</h2>
	...
	{#if course.canAdminister}
		<p class="hint">{m.classroom_staff_ownerLocked()}</p>

		<form onsubmit={add}>
			<label class="field">
				<span>{m.classroom_staff_userId()}</span>
				...
				<span class="hint">{m.classroom_staff_userIdHint()}</span>
			</label>
			<label class="field">
				<span>{m.classroom_staff_role()}</span>
				<select bind:value={newRole}>
					<option value="assistant">{m.classroom_role_assistant()}</option>
					<option value="admin">{m.classroom_role_admin()}</option>
				</select>
			</label>
			<button type="submit" class="primary">{m.classroom_staff_add()}</button>
		</form>
