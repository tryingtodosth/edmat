<script lang="ts">
	...
	import type { CourseItem } from '$lib/types/classroom';
...
<section class="queue">
	<h2>{m.classroom_review_heading()} <span class="count">{pending.length}</span></h2>

	{#if pending.length === 0}
		<p class="hint">{m.classroom_review_empty()}</p>
	{:else}
		<ul>
			{#each pending as item (item.id)}
				<li>
					<div class="row">
						<span class="kind">
							{item.kind === 'material'
								? m.classroom_items_material()
								: m.classroom_items_exercise()}
						</span>
						...
						{#if item.submittedBy}
							<span class="by">
								{m.classroom_review_by({ name: item.submittedBy.displayName })}
							</span>
						{/if}
					</div>
					...
					<div class="row">
						<input
							type="text"
							placeholder={m.classroom_review_note()}
