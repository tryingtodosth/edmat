<script lang="ts">
	...
	import type { CourseInvite, InviteRole } from '$lib/types/classroom';
	...
	const stateLabels: Record<string, () => string> = {
		revoked: m.classroom_invite_state_revoked,
		expired: m.classroom_invite_state_expired,
		used_up: m.classroom_invite_state_used_up
	};
...
<section class="invites">
	<h2>{m.classroom_invites_heading()}</h2>
	<p class="hint">{m.classroom_invites_bypassNote()}</p>

	{#if invites.length === 0}
		<p class="hint">{m.classroom_invites_empty()}</p>
	{:else}
		<ul>
			{#each invites as invite (invite.id)}
				<li class:spent={!invite.isUsable}>
					<div class="row">
						<strong>{invite.label || m.classroom_invites_heading()}</strong>
						<span class="role">{invite.role}</span>
						<span class="uses">
							{invite.maxUses
								? m.classroom_invites_usedOf({ uses: invite.uses, max: invite.maxUses })
								: m.classroom_invites_used({ uses: invite.uses })}
						</span>
					...
					<div class="row">
						...
						<button type="button" onclick={() => copy(invite)}>
							{copiedId === invite.id ? m.classroom_invites_copied() : m.classroom_invites_copy()}
						</button>
						{#if !invite.revokedAt && onrevoke}
							<button type="button" class="link danger" onclick={() => onrevoke?.(invite.id)}>
								{m.classroom_invites_revoke()}
							</button>
						{/if}
					</div>
				</li>
	...
	<form onsubmit={create}>
		<label class="field">
			<span>{m.classroom_invites_label()}</span>
			<input type="text" bind:value={label} maxlength="120" />
			<span class="hint">{m.classroom_invites_labelHint()}</span>
		</label>
		<label class="field">
			<span>{m.classroom_invites_role()}</span>
			<select bind:value={role}>
				<option value="participant">{m.classroom_invites_roleParticipant()}</option>
				<option value="assistant">{m.classroom_role_assistant()}</option>
				<option value="admin">{m.classroom_role_admin()}</option>
			</select>
		</label>
		<label class="field">
			<span>{m.classroom_invites_maxUses()}</span>
			<input type="number" bind:value={maxUses} min="0" />
			<span class="hint">{m.classroom_invites_maxUsesHint()}</span>
		</label>
		<label class="field">
			<span>{m.classroom_invites_expiresAt()}</span>
			<input type="datetime-local" bind:value={expiresAt} />
		</label>
		<button type="submit" class="primary">{m.classroom_invites_create()}</button>
	</form>
