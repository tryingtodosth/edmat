<script lang="ts">
	...
	import { acceptInvite, previewInvite } from '$lib/services/classroom';
	import type { InvitePreview } from '$lib/types/classroom';
	...
	async function accept() {
		...
		try {
			...
			done = result.detail === 'joined' ? m.classroom_join_joined() : m.classroom_join_already();
			...
			await goto(resolve('/classroom/[id]', { id: result.courseId }));
		} catch (err) {
			const detail = (err as { body?: { detail?: string } })?.body?.detail;
			error =
				detail === 'full'
					? m.classroom_join_full()
					: detail && reasonLabels[detail]
						? m.classroom_join_unusable()
	...
	let loginHref = $derived(
		`${resolve('/login')}?next=${encodeURIComponent(`/classroom/join/${token}`)}`
	);
</script>

<svelte:head>
	<title>{preview ? preview.courseTitle : m.classroom_join_heading()} — {m.common_appName()}</title>
...
<main class="join">
	...
	{:else if notFound || !preview}
		<h1>{m.classroom_join_heading()}</h1>
		<p class="error">{m.classroom_join_unknown()}</p>
	{:else}
		<h1>{m.classroom_join_heading()}</h1>
		<p class="lede">
			{m.classroom_join_body({ name: preview.instructorName, title: preview.courseTitle })}
		</p>
		<p class="hint">
			{preview.role === 'participant'
				? m.classroom_join_asParticipant()
				: m.classroom_join_asStaff()}
		</p>
		...
		{:else if !authStore.user}
			...
			<a class="primary" href={loginHref}>{m.classroom_join_login()}</a>
		{:else}
			<button type="button" class="primary" disabled={busy} onclick={accept}>
				{m.classroom_join_accept()}
			</button>
		{/if}
