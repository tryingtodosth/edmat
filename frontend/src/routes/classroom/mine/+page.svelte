<script lang="ts">
	...
	import { getMyParticipation, getMyTeaching } from '$lib/services/classroom';
	...
	let teaching = $state<TaughtCourse[]>([]);
	let participating = $state<TaughtCourse[]>([]);
...
<svelte:head>
	<title>{m.classroom_myCourses()} — {m.common_appName()}</title>
</svelte:head>

<div class="page">
	<header class="head">
		<h1>{m.classroom_myCourses()}</h1>
	...
	{:else}
		<section>
			<h2>{m.classroom_teachingHeading()}</h2>
			{#if teaching.length === 0}
				<p class="status">{m.classroom_teachingEmpty()}</p>
			{:else}
				<div class="grid">
		...
		<section>
			<h2>{m.classroom_participatingHeading()}</h2>
			{#if participating.length === 0}
				<p class="status">{m.classroom_participatingEmpty()}</p>
			{:else}
				<div class="grid">
