<script lang="ts">
	...
	import { getCourse, updateCourse } from '$lib/services/classroom';
	...
	async function submit(draft: TaughtCourseDraft) {
		...
		try {
			...
			await goto(`${resolve('/classroom')}/${page.params.id}`);
		} catch (e) {
...
<svelte:head>
	<title>{m.classroom_edit()} — {m.common_appName()}</title>
</svelte:head>
...
<div class="page">
	...
	{:else if !course.isInstructor}
		<!-- The API already refuses this; saying so here avoids presenting a form that could only
		     fail on submit. -->
		<p class="status">{m.classroom_refusal_notYours()}</p>
	{:else}
		<h1>{m.classroom_edit()}</h1>
		<CourseForm
			initial={course}
