...
- **Close the small structural gaps `CLAUDE.md` itself already named and left open**: no avatar
  ...
  folders. (`MaterialSubmission.type` not being validated is **fixed and gone**: that model was
  retired on 2026-09-22 and the project create endpoint that replaced it validates `type`,
  `audience`, the currency and every coverage topic against the branch before anything is written.)
...
| **materials** | **Stale as written** — `MaterialReview` (a star rating with a body) and a whole-material comment thread both exist now; what does NOT is any quality signal on a *version*, so a material that got worse has no way to say so beyond a report. A material's detail page also pays five queries per coverage claim (175 queries for the 32-claim corpus material), which is pre-existing and unrelated to co-authoring but real | Other-locale translations are not versioned (co-authoring versions the project's own locale; every other language keeps what it said and is only flagged as possibly stale), and a changed **file** has no diff — only text does | Requirements/price/time-estimate fields are self-declared by the uploader with no verification step — a materially wrong "prerequisite" or price is only caught if a governor happens to check. Since 2026-09-22 a material's own team publishes versions with no moderator in between (`HISTORY.md` §17BC): a deliberate open-science bet, covered by reports and a kill switch rather than by review |
